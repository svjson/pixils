...
namespace Pixils::Script
{
  namespace Function
  {
    ...
    FUNC_IMPL(MakeStyle,
              SIG((FN_ARGS((&Roo::Type::MAP)), EXEC_DISPATCH(&MakeStyle::exec_make))));

    FUNC_IMPL(MakeText,
              SIG((FN_ARGS((&Roo::Type::MAP)), EXEC_DISPATCH(&MakeText::exec_make))));

    FUNC_IMPL(MakeLayout,
              SIG((FN_ARGS((&Roo::Type::MAP)), EXEC_DISPATCH(&MakeLayout::exec_make))));
    ...
    EXEC_BODY(MakeText, exec_make)
    {
      ...
      if (!text) return Roo::Constant::NIL;
    ...
    EXEC_BODY(MakeLayout, exec_make)
    {
      ...
      if (!layout) return Roo::Constant::NIL;
    ...
    EXEC_BODY(MakeStyle, exec_make)
    {
      ...
      if (!style) return Roo::Constant::NIL;
    ...
    FUNC_IMPL(MakeBorder,
              SIG((FN_ARGS((&Roo::Type::MAP)), EXEC_DISPATCH(&MakeBorder::exec_make))));

    EXEC_BODY(MakeBorder, exec_make)
    {
      auto border = StyleDefinition::build_border(ctx, args[0]);
      if (!border) return Roo::Constant::NIL;
    ...
    FUNC_IMPL(MakeInsets,
              MULTI_SIG((FN_ARGS((&Roo::Type::NUMBER)),
                         EXEC_DISPATCH(&MakeInsets::exec_make_num)),
                        (FN_ARGS((&Roo::Type::VECTOR_OF_NUMBER)),
    ...
    EXEC_BODY(MakeInsets, exec_make_map)
    {
      ...
      if (!insets) return Roo::Constant::NIL;
  ...
  StyleNamespace::StyleNamespace()
    : Roo::Namespace(std::string(NS__PIXILS__UI__STYLE))
  {
