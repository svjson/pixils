...
TEST_F(TilemapEditorStartupTest, terrain_rule_add_button_creates_visible_rule)
{
 ...
 :tilemap {:width 2
  ...
  auto exclusive_checkbox = find_descendant_mode_containing_state(session.active_mode,
                                                                  "ui/checkbox",
                                                                  ":terrain-rule-exclusive");
  ASSERT_NE(exclusive_checkbox, nullptr);
  input().mouse_down({exclusive_checkbox->bounds.x + exclusive_checkbox->bounds.w / 2,
                      exclusive_checkbox->bounds.y + exclusive_checkbox->bounds.h / 2});
  update_cycle();
  input().mouse_up({exclusive_checkbox->bounds.x + exclusive_checkbox->bounds.w / 2,
                    exclusive_checkbox->bounds.y + exclusive_checkbox->bounds.h / 2});
  update_cycle();
  update_cycle();
  EXPECT_NE(session.active_mode->state->to_string().find(":exclusive? true"),
            std::string::npos);
...
TEST_F(TilemapEditorStartupTest, terrain_rule_exclusive_checkbox_reflects_loaded_rule)
{
  const auto history_path = std::filesystem::temp_directory_path() /
                            "pixils-tilemap-editor-exclusive-checkbox-history.edn";
  const auto project_path = std::filesystem::temp_directory_path() /
                            "pixils-tilemap-editor-exclusive-checkbox-project.edn";
  std::error_code ec;
  std::filesystem::remove(history_path, ec);
  std::filesystem::remove(project_path, ec);
 ...
 :tilemap {:width 2
  ...
  session.pop_mode(runtime.eval(R"({:type :confirm
                                ...
                                lisp_string(project_path.string()) + R"(
                                  :directory )" +
                                lisp_string(project_path.parent_path().string()) + R"(
                                  :filename "exclusive-checkbox-project.edn"})"));
  ...
  auto exclusive_checkbox = find_descendant_mode_containing_state(session.active_mode,
                                                                  ...
                                                                  ":terrain-rule-exclusive");
  ASSERT_NE(exclusive_checkbox, nullptr);
  SCOPED_TRACE(exclusive_checkbox->state->to_string());
  EXPECT_NE(exclusive_checkbox->state->to_string().find(":checked? true"),
            std::string::npos);
