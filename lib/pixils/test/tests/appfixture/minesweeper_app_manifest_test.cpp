...
TEST(MinesweeperAppManifestTest, materializes_pre_windowed_catalog)
{
  auto files = Minesweeper::pre_windowed_manifest().materialize_files();
...
TEST(MinesweeperAppManifestTest, materializes_simple_windowed_catalog)
{
  ...
  EXPECT_EQ(core->units[0].id, Minesweeper::unit_ids::program);
  EXPECT_EQ(core->units[1].id, Minesweeper::unit_ids::status_bundle);
  EXPECT_EQ(core->units[2].id, Minesweeper::unit_ids::counter_bundle);
  EXPECT_EQ(core->units[3].id, Minesweeper::unit_ids::status_panel_left_pad);
  EXPECT_EQ(core->units[4].id, Minesweeper::unit_ids::counter_component);
  EXPECT_EQ(core->units[5].id, Minesweeper::unit_ids::status_panel_component);
  EXPECT_EQ(core->units[6].id, Minesweeper::unit_ids::mine_layer_render);
  EXPECT_EQ(core->units[7].id, Minesweeper::unit_ids::mine_layer_mode);
  EXPECT_EQ(core->units[8].id, Minesweeper::unit_ids::board_button_component);
  EXPECT_EQ(core->units[9].id, Minesweeper::unit_ids::board_button_row_component);
  EXPECT_EQ(core->units[10].id, Minesweeper::unit_ids::board_buttons_component);
  EXPECT_EQ(core->units[11].id, Minesweeper::unit_ids::board_mode);
  EXPECT_EQ(core->units[12].id, Minesweeper::unit_ids::highscore_row_component);
  EXPECT_EQ(core->units[13].id, Minesweeper::unit_ids::highscore_table_component);
  EXPECT_EQ(core->units[14].id, Minesweeper::unit_ids::highscore_buttons_component);
  EXPECT_EQ(core->units[15].id, Minesweeper::unit_ids::highscores_window_component);
  EXPECT_EQ(core->units[16].id, Minesweeper::unit_ids::highscores_modal_mode);
  ...
  const auto* shared_button =
    find_file(files, "pixils.test.app.shared.ui.components.button");
  ASSERT_NE(shared_button, nullptr);
  ASSERT_EQ(shared_button->units.size(), 2u);
  EXPECT_EQ(shared_button->units[0].id, Minesweeper::unit_ids::button_inner_component);
  EXPECT_EQ(shared_button->units[1].id, Minesweeper::unit_ids::button_component);
